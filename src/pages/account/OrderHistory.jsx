            ...
            {modalType === 'return' && (
              <form onSubmit={handleSubmitReturn} className="space-y-4">
                ...
                <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 text-xs text-orange-800 leading-relaxed font-medium">
                  Sertakan video unboxing melalui WhatsApp agar proses retur tervalidasi dan diproses lebih cepat.
                </div>
                ...
                <div className="pt-4 flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-4 my-1">
                    ...
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">ATAU LEBIH CEPAT</span>
                    <div className="h-px bg-gray-200 flex-1"></div>
                  </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const snapshot = parseSnapshot(selectedOrder?.shippingSnapshot);
                      const buyerName = snapshot?.recipientName || user?.name || "Customer";
                      const orderDate = new Date(selectedOrder?.createdAt).toLocaleDateString('id-ID');
                      
                      const text = `Halo admin, saya ingin mengajukan retur.\n\nNo. Resi: ${trackingNumber}\nPesanan: ${returnItem || "-"}\nNama Pembeli: ${buyerName}\nTanggal Pembelian: ${orderDate}\nDetail Kendala: ${returnDetail || "-"}\n\nMohon dibantu untuk proses validasi retur. Terima kasih.`;
                      
                      window.open(`https://wa.me/6282110737267?text=${encodeURIComponent(text)}`, '_blank');
                    }}
                    className="w-full p-3.5 bg-[#25D366] hover:bg-[#128C7E] shadow-sm text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2"
                  >
